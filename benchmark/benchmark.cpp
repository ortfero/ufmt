...
int main() {
    ...
    auto const snprintf_char =
        ubench::run([&] { snprintf(charz, sizeof(charz), "%c", 'a'); });
    auto const fmt_char =
        ubench::run([&] { fmt::format_to(charz, "{}", 'a'); });
    ...
    auto const fmt_literal =
        ubench::run([&] { fmt::format_to(charz, "{}", "some literal"); });
    ...
    auto const snprintf_int =
        ubench::run([&] { snprintf(charz, sizeof(charz), "%d", -127562); });
    auto const fmt_int =
        ubench::run([&] { fmt::format_to(charz, "{}", -127562); });
