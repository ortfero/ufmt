...
namespace ufmt {
    ...
    public:
        ...
        friend basic_text& operator<<(basic_text& self,
    ...
    Stream& operator<<(Stream& stream, basic_text<String> const& bt) {
        return stream << bt.string();
    }


    template<class S>
    basic_text<S>& operator<<(basic_text<S>& self, typename S::value_type ch) {
...
#else
            auto const n =
                std::snprintf(p, float_digits, "%.*f", f.precision, f.value);
            if(n <= 0)
                self.free(p);
            else
                self.free(p + n);
    ...
    formatters::integer_fixed<std::uint64_t> fixed(std::uint64_t value,
                                                   int width) {
        return formatters::integer_fixed<std::uint64_t> {value,
                                                         std::size_t(width)};
    }
